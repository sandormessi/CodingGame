...
public class Game
{
   ...
   private static Coordinate2D CalculateCenterOfTargetSquare(Coordinate2D actualPosition,
      IReadOnlyList<Coordinate2D> positionsSoFar, Directions direction, int buildingWidth, int buildingHeight)
   {
      ...
      switch (direction)
      {
         ...
         case Directions.L:
            ...
            corner = new Coordinate2D(
               positionsSoFar.Where(p => p.X < actualPosition.X).MaxBy(p => p.X)?.X ?? 0,
               actualPosition.Y);
         ...
         case Directions.R:
            ...
            corner = new Coordinate2D(
               positionsSoFar.Where(p => p.X > actualPosition.X).MinBy(p => p.X)?.X ?? buildingWidth - 1, 
               actualPosition.Y);
